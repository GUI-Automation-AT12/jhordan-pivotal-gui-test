...
public class ProfileSteps {
    ...
    // Entities
    private User user, userToRestore;
    ...
    /**
     * displays a message the section
     * @param message
     */
    @Then("{string} message should be displayed in the section")
    public void displayedSection(String message) {
        String actual = profilePage.getTextFromErrorMessage();
        assertEquals(message, actual);
    }

    /**
     * The User name should not be updated in the management menu.
     * @param type
     */
    @Then("my {string} should not be updated in the User Management Menu")
    public void shouldNotBeUpdatedInTheUserManagementMenu(String type) {
        for (String userAlias : context.getEditedUsersList()) {
            ProfilePage profilePage = new ProfilePage();
            userToRestore = context.getUserByAlias(userAlias);
        }
        SoftAssert softAssert = new SoftAssert();
        softAssert.assertEquals(userToRestore.getUserName(),profilePage.getActualInformation(type));
